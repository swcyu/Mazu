...
# 하둡에서 데이터 가져오기
df_1 = spark.read.format("csv").option("header","true").option("multiline","true").option("quote", "\"").option("escape","\"").load("yogi6/raw_data/crawling/init/mcst_1.csv")
...
# 사용할 칼럼만 선택
df_1 = df_1.select('exhibition_name', 'poster_link', 'exhibition_description', 'exhibition_period', 'exhibition_place')
...
df_final.write.format('parquet').mode('overwrite').save("file:///home/ubuntu/yogi6/tr_data/crawling/init/mcst.parquet")
#1차 가공된 파일
#df_final.write.format('parquet').mode('overwrite').save("hdfs:///user/ubuntu/yogi6/tr_data/crawling/mcst/init/mmca.parquet")
df_final.write.format('parquet').mode('overwrite').save("hdfs:///user/ubuntu/yogi6/tr_data/crawling/init/mcst.parquet")