...
df_pre = spark.read.format("csv").option("header", "true").option("multiline", "true").option("quote", "\"").option(
    "escape", "\"").load("yogi6/raw_data/crawling/init/museum_pre.csv")
df_past = spark.read.format("csv").option("header", "true").option("multiline", "true").option("quote", "\"").option(
    "escape", "\"").load("yogi6/raw_data/crawling/init/museum_past.csv")
...
df = df_pre.union(df_past) \
    ...
    .drop(col('exhibition_period')) \
    .withColumn('place', lit('국립중앙박물관')) \
...
##파일 저장##
# 분석, 리뷰크롤링용 csv파일
#pd_df.to_csv('~/yogi6/ds/file/museum.csv', encoding="utf-8-sig")
...
# 배포용 파일
#df_final.write.format('parquet').mode("overwrite").save("file:///home/ubuntu/yogi6/tr_data/crawling/museum/init/museum.parquet")
df_final.write.format('parquet').mode("overwrite").save("file:///home/ubuntu/yogi6/tr_data/crawling/init/museum.parquet")
# 1차 가공된 파일
#df_final.write.format('parquet').mode("overwrite").save("hdfs:///user/ubuntu/yogi6/tr_data/crawling/museum/init/museum.parquet")
df_final.write.format('parquet').mode("overwrite").save("hdfs:///user/ubuntu/yogi6/tr_data/crawling/init/museum.parquet")