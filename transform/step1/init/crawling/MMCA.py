...
df_pre = spark.read.format("csv").option("header", "true").option("multiline", "true").option("quote", "\"").option(
    "escape", "\"").load("yogi6/raw_data/crawling/init/mmca_pre.csv")
...
'''
# 사용할 칼럼만 선택
df_pre = df_pre.select('exhibition_name', 'poster_link', 'exhibition_description', 'exhibition_period', 'exhibition_place')
...
#df_final.write.format('parquet').mode('overwrite').save("hdfs:///user/ubuntu/yogi6/tr_data/crawling/mmca/init/mmca.parquet")
