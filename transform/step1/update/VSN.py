...
df_pre = spark.read.format("csv").option("header", "true").option("multiline", "true").option("quote", "\"").option(
    "escape", "\"").load(f"yogi6/raw_data/crawling/{now}/VSN.csv")
...
#print(df_final.printSchema(), df_final.count())
...
df_final.write.format('parquet').mode("overwrite").save(f"hdfs:///user/ubuntu/yogi6/tr_data/crawling/{now}/vsn.parquet")